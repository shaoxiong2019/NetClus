...
public class RankFunc {
	...
	/**
	 * 
	 * @param ox order and x type adjacency matrix 
	 * @param tl cluster target list (id)
	 * @return
	 */
	...
	public Map<String, Matrix> AuthorityRank(ArrayList<Integer> tl, int k) {
		...
		Matrix rim = moi.transpose().times(doc.inverse()).times(moc)
				.times(moc.transpose()).times(doi.inverse()).times(moi);
		Matrix rcm = moc.transpose().times(doi.inverse()).times(moi)
				.times(moi.transpose()).times(doc.inverse()).times(moc);
